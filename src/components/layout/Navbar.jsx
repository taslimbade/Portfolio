...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-1000 w-full transition-all duration-300 ${isScrolled ? 'bg-black/30 backdrop-blur-lg' : 'bg-transparent'}`} 
        ...
        <nav className="hidden md:flex items-center gap-7">
          {Nav_LINKS.map(link => (
            <button
              ...
              className={`text-base font-medium transition-all duration-300 ${activeSection === link.id ? 'text-white'
    ...
    <div 
      ...
      >
        <div className="bg-black backdrop-blur-lg border-t border-white/10 px-5 py-6 space-y-3">
          {Nav_LINKS.map((link) => (
            <button
              key={link.id}
              onClick={() => handleNavClick(link.id)}
